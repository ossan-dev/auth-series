# Support two authentication provider with .NET 5
...
1. A project within Google Firebase with a test user who can sign-in with username and password
1. Built a web api project that makes use of Jwt authentication
1. Integrate our Firebase proj within our web api so users can access protected resources (after a successful sign-in on Firebase)
1. Implement our custom sign-in mechanism by exposing an endpoint to issue a Jwt token to users  

👀*NOTE*👀: if you've not followed the previous posts but you have a solution similar to the one presented here (maybe different auth providers) the following approach still works for you even if you probably have to make some adjustements.  
## Final challenge 🎯
During this post we're going to manage two authentication providers in the same web api at the same time. What this means is that a user can choice how to sign-in against our web api. The user should authenticates himself against Google Firebase or against our custom endpoint. If the sign-in phase is successful the user will got back a valid Jwt token which can be used to access protected resources. As before, if the sign-in is not successful he'll got back an error response and cannot access our super-secure resources 😎.
## Requirements
...
## Let's start 🚀
To complete the task we've to carry about two sections: the settings and authentication's registration. After these duties we'll do the final test. 
## Check settings 🔧
...
After the Firebase registration you have to adjust the code you have in a way that looks similar to the one below:  
```csharp
    ...
...
```
The main change in this code is that we're going to use another overload of the method _AddJwtBearer_. This overload accepts as the first parameter the authentication schema name which is used to identify the providers univoquely.  
...
The last step left is to add a policy to our web api that allow it to manage multiple authentication schemas at the same time. This policy must be written right below the code in the previous secion. So open up "Startup.cs" and locate the "ConfigureServices()" method and write the code:  
```csharp
...
```
This code snippet is self-explanatory. Its only purpose is to register the schemas declared above as you should notice from the command `.AddAuthenticationSchemes("Firebase", "AuthDemo")`.
## Final Test 🐲
We've finished the coding phase so we can finally give a try to our work. In your terminal go in the folder where it's contained the .csproj file.  
Issue a `dotnet run` command and wait for the web api to start properly.
### Postman
To test our software we need of three requests (that you can build by following the recipes of the previous posts):
...
## Final thoughts 💭
Now we reached the end 🏁. During this series you learned a bunch of things about authentication within .NET 5. To show off these capabilities we make use of web api project but the same still applies to other projs such as MVC or Single Page Application.  
Supporting multiple authentication providers could be a transient phase such as when you're migrating from one auth provider to another or permament as you would like to let users decide which authentication provider utilize.  
In both cases this series will provide you some guidelines about the steps you have to follow.  

## Greetings 👋🏻
Now, it's time to say goodbye 😄.  
**🔵IMPORTANT🔵**: remember that authentication is one **CRUCIAL** aspect of software development, so don't joke with it as you'll pay the consequencies for sure 🤕  
